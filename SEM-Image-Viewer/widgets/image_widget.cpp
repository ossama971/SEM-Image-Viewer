...
ImageWidget::ImageWidget(QWidget *parent)
  ...
  // connect(imagerepo, &ImageRepository::onImageChanged, this,
  //         &ImageWidget::reload);
  // connect(imagerepo->getImage(), &Image::onImageStateUpdated, this,
  //         &ImageWidget::reload, Qt::UniqueConnection);
}
...
void ImageWidget::showEvent(QShowEvent *event) {
  ...
  // QString image_path =
  // QString::fromStdString(imagerepo->getImage()->getPath().string());
...
void ImageWidget::loadAndDisplayImage(const Image &image) {
  ...
  if (pixmap) {
    setImage(*pixmap);
    infoBar->setDimensions(pixmap->width(),
                           pixmap->height()); // Set image dimensions
  } else {
...
optional<QPixmap> ImageWidget::loadAndPrepareImage(const Image &selected_image,
  ...
  QImage qImage = QImage(image.data, image.cols, image.rows, image.step[0],
